 ...
 ****************************************************/
 ...
 // WARNING : Modifications made by Ethan Syed
 // No guarantee that code functions as originally written by Jonathan W. Valvano
 // Additional functions :
 // 	- Text Scaling
 //		- Text color change (ST7735_OutString)
 //		- X & Y Buffers for Text
 //
...
// Added by : Ethan S
uint8_t textSize = 1;
uint8_t xCharCap[10] = {25, 12 , 9, 7, 5, 5, 4, 4, 3, 3};
uint8_t yCharCap[10] = {15, 7, 5, 4, 3, 2, 2, 2, 1, 1};
int32_t XBuffer = 0;
int32_t YBuffer = 0;
uint8_t reflectMode = 0;
		...
		for (i = 5; i >= 0; i--) {
				if (i == 0)
					line = 0x0;
					 ...
					 line = Font[(c*5)+(5-i)]; // Access font data in reverse order
				for (j = 0; j < 8; j++) {
					if (line & (1 << j)) {
						if (size == 1) // default size
							ST7735_DrawPixel(x + i, y + j, textColor); // Draw the reflected pixel
						else {  // big size
							ST7735_FillRect(x + (i * size), y + j * size, size, size, textColor); // Draw the reflected pixel
						}
					} else if (bgColor != textColor) {
						if (size == 1) // default size
							ST7735_DrawPixel(x + i, y + j, bgColor); // Draw the reflected background pixel
						else {  // big size
							ST7735_FillRect(x + (i * size), y + j * size, size, size, bgColor); // Draw the reflected background pixel
						}
					}
				}
			}
...
uint32_t ST7735_DrawString(uint16_t x, uint16_t y, char *pt, int16_t textColor){
  ...
  if(y>15) return 0;
  while(*pt){
    ST7735_DrawCharS(XBuffer + (x*6*textSize), YBuffer + (y*10*textSize), *pt, textColor, ST7735_BLACK, textSize);
    pt++;
    x = x+1;
...
void ST7735_SetCursor(uint32_t newX, uint32_t newY){
  if((newX > xCharCap[textSize-1]) || (newY > yCharCap[textSize-1])){       // bad input
    return;                             // do nothing
  }
	if(reflectMode){
		StX = xCharCap[textSize-1] - newX;
		StY = newY;
	}else{
  StX = newX;
  StY = newY;
...
void ST7735_OutUDec(uint32_t n){
  ...
  if(StX>25){
    StX = 25;
    ST7735_DrawCharS(XBuffer + (StX*6*textSize), YBuffer + (StY*10*textSize),'*',ST7735_RED,ST7735_BLACK, textSize);
  }
}
...
void ST7735_OutUDec4(uint32_t n){
  ...
  if(StX>25){
    StX = 25;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, textSize);
  }
}
//-----------------------ST7735_OutUDec5-----------------------
...
void ST7735_OutChar(char ch){
  ...
  ST7735_DrawCharS(XBuffer + (StX*6*textSize), YBuffer + (StY*10*textSize),ch,StTextColor,ST7735_BLACK, textSize);
  StX = (reflectMode)? StX-1 : StX+1;
  if(StX>25){
    StX = 25;
    ST7735_DrawCharS(StX*6*textSize,StY*10*textSize,'*',ST7735_RED,ST7735_BLACK, textSize);
  }
  return;
}
//********ST7735_OutString*****************
...
void ST7735_IncXBuf(int32_t amount){
	XBuffer = (XBuffer+amount > 50)? 50 : XBuffer+amount;
}

void ST7735_IncYBuf(int32_t amount){
	YBuffer = (YBuffer+amount > 50)? 50 : YBuffer+amount;
}
void ST7735_Reflect(void){
	reflectMode = (reflectMode)? 0 : 1;
}