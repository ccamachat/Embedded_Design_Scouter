...
void UART_OutBits(uint8_t data) {
    // Loop through each bit of the uint8_t value
    for (int i = 7; i >= 0; i--) {
        // Extract the i-th bit and send it to UART
        char bit = (data >> i) & 0x01; // Extract the i-th bit
        UART_OutChar(bit ? '1' : '0'); // Send '1' if bit is set, '0' otherwise
    }
}

void UART_OutInt16(int16_t data) {
    char buffer[20]; // Assuming 20 characters is sufficient for the string representation
